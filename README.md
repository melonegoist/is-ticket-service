...
1. **Фаза подготовки** — файл читается в память и загружается в MinIO. После успешной загрузки операция импорта переводится в статус `PREPARED` с сохранением ключа объекта и оригинального имени файла. При недоступности MinIO импорт прерывается до взаимодействия с БД.
...
- **Два администратора одновременно редактируют один билет:** транзакции на обновление выполняются в `SERIALIZABLE`, поэтому одна из операций получает конфликт/rollback, а клиент видит актуальные данные после повторной загрузки списка.
- **Один пользователь удаляет билет, пока другой меняет его цену:** удаление и обновление проходят в отдельных транзакциях с повышенной изоляцией; при гонке обновление получает исключение «не найдено» или конфликт и не приводит к частичному обновлению.
- **Параллельные создания сущности с уникальным значением (паспорт):** сервис проверки + уровень `SERIALIZABLE` предотвращают «двойную вставку», одна из транзакций откатывается, сохраняя уникальность.
